...
class Main:
    def __init__(self,adminemail):
        ...
        self.mainLabel=tkinter.Label(self.root,text='Change Password',font=('',28,'bold'),bg="#6E726E",fg='white',relief=tkinter.SUNKEN)
        ...
        self.txt2 = tkinter.Entry(self.formFrame,font=('arial',14),show="*")
        ...
        self.txt3 = tkinter.Entry(self.formFrame, font=('arial', 14),show="*")
        self.lb3.grid(row=2, column=0, pady=10, padx=10)
        self.txt3.grid(row=2, column=1, pady=10, padx=10,ipadx=10,ipady=10)

        self.bt1=tkinter.Button(self.root,text='Submit',font=('arial',14),command=self.insertadmin,bg='#CC2936',foreground='black',activebackground='black',activeforeground="white"
    ...
    def insertadmin(self):
        ...
        if len(result) == 0:
            ...
            cr.execute(q)
            conn.commit()
            msg.showinfo('', 'Admin Password has been Updated', parent=self.root)
            self.root.destroy()
